...
The following decision trees predicts the importance of teaching children tolerance based on 2 immigration viewpoints: friendliness towards immigrants as neighbors, and whether immigration increases the risk of terrorism. 
```{r}
# sub-setting data
tree_subset <- 
  wvs %>% 
...
* Within the much smaller cohort that responded that they wouldn't like having immigrants as neighbors - *22%* - 10% are neutral or disagree with the question of whether immgiration increases the risk of terrorism. I'll include this group in the somewhat supportive of immigration category. This group also believes in teaching tolerance to children, though by a slim margin of about 6%. 
* In contrast, 12% of respondents agree that immgiration increases the risk of terrorism, and responded negatively to the idea of being neighbors with immigrants. This group is classified as the most infavorable to immigration. Still, 59% of this group believes in teaching tolerance to children.

###### Based on this classification system, respondents fall into the following categories regarding immigration viewpoints: 27% are very supportive, 44% are somewhat supportive, 18% are neutral or unsure, and 12% are not supportive.
...
religion_tree <- 
  ...
  select(
    # --------------------------- PREDICTING RELIGION ---------------------------
    Religion,
    # ------------------- GIVEN SOCIAL VIEWS / ETHICAL VALUES -------------------
    terrorism,
    death.penalty,
    suicide,
    beating.wife,
...
fancyRpartPlot(religion_tree_model, 
               main = "Predicting Religious Denomination\nBased on Social Views and Ethical Values",
               caption = "0 = None\n1 = Roman Catholic\n5 = Muslim",
               type = 5)
```
...
```{r}
#Testing the model
pred_religion_tree_model <- predict(object = religion_tree_model, religion_tree[-1], type = "class")
...
### Business Question #3: Predicting the happiness of an individual given demographic information.

... talk about reason for choosing decision tree for this particular question ...

```{r}
# Predicting the happiness of an individual.
happiness_tree <- 
  wvs_subset %>% 
  select(
    # ----------------------- PREDICTING HAPPINESS -----------------------
    Income.Group,
    # ---------------- GIVEN DEMOGRAPHICAL INFORMATION ----------------
    votes.locally,
    votes.nationally,
...
set.seed(888)
# remove NAs
happiness_tree <- na.omit(happiness_tree)

# partition
happiness_tree_partition <- createDataPartition(y = happiness_tree$Income.Group, p = 0.8, list = FALSE)
happiness_tree_train <- na.omit(happiness_tree[happiness_tree_partition, ])
happiness_tree_test <- happiness_tree[-happiness_tree_partition, ]

# building the classification tree with rpart
happiness_tree_model <- rpart(formula = Income.Group ~ .,
                              data = happiness_tree_train,
                              method = "class")

summary(happiness_tree_model)

# Visualize the decision tree with rpart.plot
fancyRpartPlot(happiness_tree_model, 
