...
```{r}
# table(wvs_subset$Age, wvs_subset$imm.brings.social.conflict)
# prop.table(table(wvs_subset$Age))

# tables & correlation
# data.frame(colnames(wvs_subset))
# NOTE FROM JUSTIN: this is likely different now that our subsets have been combined/rearranged
# You'll likely need to change the values here if you want to include this. 
# Alternatively, reference the visualization (just strength of colors) instead of specific p-value
# wvs_subset3 <- wvs_subset[c(-1,-2,-3,-7, -11, -12)]
# str(wvs_subset3)
